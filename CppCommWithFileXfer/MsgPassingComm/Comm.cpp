...
const std::string saveClientFilePath = "../CppCommWithFileXfer/ClientSaveFiles";
const std::string sendclientFilePath = "../../../../CppCommWithFileXfer/ClientSendFiles";
...
void Sender::start()
{
  std::function <void()> threadProc = [&]() {
    while (true)
    {
      ...
      if (msg.to().address != lastEP.address || msg.to().port != lastEP.port || msg.command() == "estConnection" )
      {
        connecter.shutDown();
        //connecter.close();
...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string fileSpec;
  if (msg.command().compare("checkin")==0)
  {
	  fileSpec = sendclientFilePath + "/" + msg.file();
  }
  else
  {
  ...
  */
  bool receiveFile(Message msg)
  {
	...
	if (msg.command().compare("checkin")==0)
	{
		...
		fileSpec = saveClientFilePath + "/" + fileName;

	}
	else
	{
		fileSpec = saveFilePath + "/" + fileName;
	}
