...
namespace Repository
{
  ...
  class RepoCoreProvider
  {
  public:
	  static RepositoryCore<PayLoad>* getRepoCore()
	  {
		  if (rc_ == NULL)
		  {
			  rc_ = new RepositoryCore<PayLoad>();
			  return rc_;
		  }
		  else
		  {
  ...
  private:
	  static RepositoryCore<PayLoad> *rc_;
  };
  RepositoryCore<PayLoad>* RepoCoreProvider::rc_ = NULL;

  //Server class for handling server functionality
  class Server
  {
  public:
  ...
  private:
	...
	std::vector<NoSqlDb::DbElement<PayLoad>> dbElemList;
      ...
      while (true)
      {
        ...
        if (msg.to().port != msg.from().port)  // avoid infinite message loop
        {
		  ...
		  if (reply.containsKey("file")){
			  Msg tempReply;
			  tempReply.to(msg.from());tempReply.from(msg.to());tempReply.command("fileSent");
			  tempReply.attribute("fileName", reply.value("file"));
			  if (reply.containsKey("childCount"))
				  tempReply.attribute("childCount", reply.value("childCount"));
				if (reply.containsKey("isChildRequest")) {
				  tempReply.attribute("isChildRequest", reply.value("isChildRequest"));
				  }
